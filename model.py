...
class encoder_rnn(nn.Module):
    ...
    def __init__(self, tau, delta_t, embedding_dim, noise_sigma, gain=1.0, p=1.0, encoder_bias=False, encoder_max_rank=-1):
        super().__init__()
        save_args(vars())
        self.init()
...
class linear_decoder_rnn(nn.Module):
    ...
    def __init__(self, decoder_dim, decoder_bias=False, decoder_max_rank=-1):
...
class encoder(nn.Module):
    ...
    def __init__(self, tau, delta_t, embedding_dim, noise_sigma, input_dim, input_gain=1.0, embedding=None, is_embedding_fixed=True, encoder_bias=False, encoder_max_rank=-1, timer=None):
        ...
        self.rnn = encoder_rnn(tau, delta_t, embedding_dim, noise_sigma, encoder_bias=encoder_bias, encoder_max_rank=encoder_max_rank)
...
class decoder(nn.Module):
    ...
    def __init__(self, decoder_dim, encoder_dim, input_dim, output_dim, input_embedding, linear_decoder=True, decoder_bias=None, encoder_to_decoder_equal_space=False, decoder_max_rank=-1, timer=None):
        ...
        if linear_decoder:
            if decoder_bias is None:
                decoder_bias = False
            self.rnn_cell = linear_decoder_rnn(decoder_dim, decoder_bias=decoder_bias, decoder_max_rank=decoder_max_rank)
        else:
